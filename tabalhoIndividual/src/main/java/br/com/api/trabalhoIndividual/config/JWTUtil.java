...
public class JWTUtil {
	...
	public String generateTokenWithUsuarioData(Pessoa user) throws IllegalArgumentException, JWTCreationException {
		...
		return JWT.create().withSubject(subject).withClaim("pessoa", userJson).withIssuedAt(new Date())
				.withIssuer(companyProjectName).sign(Algorithm.HMAC256(secret));
	}
	...
	public String validateTokenAndRetrieveSubject(String token) throws Exception {
		...
		try {
			user = mapper.readValue(jwt.getClaim("pessoa").asString(), Pessoa.class);
		} catch (JsonProcessingException e) {
